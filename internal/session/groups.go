...
//
// This is the fork's default in-group order (overrides upstream #857). It is
// what NewGroupTree uses, so the upstream group_sort=actionable mode below stays
// inert in the fork's tree render path by design. The "surface the actionable
// sessions" behavior #857 provided is still available on demand via the
// active-on-top view mode (the 't' toggle), which floats active sessions/groups
// to the top at render time without disturbing the stable base order. (local fork)
...
// until SetGroupSortMode is first called. (Upstream #857-configurable sort; the
// fork's NewGroupTree uses SortInstancesByOrder instead, so this only drives the
// standalone SortInstancesByActionable helper + its tests.)
