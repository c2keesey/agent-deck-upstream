...
func (i *Instance) ensureClaudeSessionIDFromDisk() {
	// Issue #1465 / #1147 (Start-path twin): an explicit `--session-id <uuid>`
	// baked into i.Command is the user's authoritative declaration of WHICH
	// conversation this session owns; adopt it before the #608 gate and disk
	// discovery so N sessions recovering in one shared cwd — each with its own
	// --session-id — can't all disk-discover the newest sibling JSONL by mtime
	// and converge onto a single id (which then leaks across title-sync and
	// trips the duplicate-session sweeper). The Restart prelude already does
	// this via the same helper; this closes the gap on the Start path.
